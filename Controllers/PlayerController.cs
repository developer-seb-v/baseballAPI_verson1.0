...
namespace baseballAPI.Controllers
{
    ...
    public class PlayerController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddPlayerObject([FromBody] Player player)
        {
            try
            {
                ...
                string query = $"INSERT INTO `player` " + "(player_number, first_name, last_name, pos_id, country_id)" +
    ...
    {
        MySqlConnection conn = new MySqlConnection(_connection);
        string query = "UPDATE `player` " + "SET first_name = @fn, last_name = @ln, pos_id= @pos, country_id = @cid " +
    ...
    public List<Country> GetCountries()
    {
        ...
        try
        {
            ...
            // created join query to get foreign table data
            string get =
                "SELECT country.country_id, country.country_name FROM country";
            MySqlCommand cmd = new MySqlCommand(get, mySqlConnection);
            MySqlDataReader reader = cmd.ExecuteReader();
            while (reader.Read())
            {
